...
export default {
  ...
  methods: {
    ...
    smoothScroll(id) {
      if (this.$route.path === `/user/profile/${this.userData.id}`) {
        this.$router.push("/");
      }
...
<template>
    <a class="flex items-center" href="#">
        ...
        <span class="text-surface-900 dark:text-surface-0 font-medium text-xl leading-normal mr-10">Car Wash</span>
    </a>
    <Button
        class="lg:!hidden"
        text
    ...
    <div class="items-center bg-surface-0 dark:bg-surface-900 grow justify-between hidden lg:flex absolute lg:static w-full left-0 top-full px-12 lg:px-0 z-20 rounded-border">
        ...
        <div class="flex border-t lg:border-t-0 border-surface py-4 lg:py-0 mt-4 lg:mt-0 gap-2">
          ...
          <i class="pi pi-spin pi-spinner text-green-500" style="font-size: 2rem" v-if="loading"></i>

          <button v-if="userData" @click="loginDrop" ref="loginMenu" class="nav-link">
            <strong class="text-green-500">{{ userData.name }}</strong> <i :class="arrowStyle" style="margin-left: 0.5rem"></i>
          </button>
          <div class="login-panel-parrent mt-7 m-0" v-if="showLoginPanel">
            <div class="login-panel ">
              <div class="login-panel__child text-center">
                <button v-if="isAuthenticated" @click="profilePage(userData.id)">Profile</button>
<!--                <router-link :to="'user/profile/' + userData.id" class="nav-link" @click="loginDrop">Profile</router-link>-->
              </div>
              <div class="login-panel__child text-center">
          ...
          <button  v-if="!userData && !loading">
            <router-link to="/login" class="mr-4 px-0 text-green-500 py-4 text-surface-900 dark:text-surface-0 font-medium text-lg">Login</router-link>
          </button>
          <button  v-if="!userData && !loading" class="bg-green-400 p-4 pt-2 pb-2" style="border-radius: 36px">
            <router-link to="/register" class="px-0 text-white py-4 text-surface-900 dark:text-surface-0 font-normal text-lg">Register</router-link>
          </button>
